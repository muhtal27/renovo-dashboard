...
async def list_cases(
    ...
    return await service.list_cases(tenant_id)
...
async def get_case_workspace(
    ...
    service = EOTService(session)
    try:
        return await service.get_case_workspace(tenant_id, case_id)
...
async def create_evidence(
    ...
    service = EOTService(session)
    try:
        return await service.add_evidence(payload)
    except LookupError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def upsert_issue(
    ...
    service = EOTService(session)
    try:
        return await service.upsert_issue(payload)
    except LookupError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
