...
function IntegrationCard({ i }: { i: Integration }) {
  return (
    <div className="group relative rounded-xl border border-white/10 bg-white/[0.03] p-5 transition-all hover:-translate-y-0.5 hover:border-emerald-500/30 hover:shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-lg border border-white/10 bg-white/[0.03] font-mono text-[11px] font-semibold tracking-tight text-white/75">
          {i.logo}
        </div>
        <StatusBadge status={i.status} />
      </div>
      <div className="mt-4 text-[15px] font-semibold leading-tight text-white">{i.name}</div>
      <div className="mt-1 text-[11px] font-medium uppercase tracking-[0.08em] text-white/45">{i.kind}</div>
      <p className="mt-3 text-[13px] leading-relaxed text-white/55">{i.desc}</p>
    </div>
  )
}
      ...
      <div className="page-shell page-stack">
        ...
        <section className="page-hero">
          ...
          <h1 className="page-title max-w-[820px]">
            Connects to the tools <em className="text-white/45">you already run.</em>
          </h1>
        ...
        <section className="mx-auto max-w-[1200px] px-6 py-16">
          <div className="space-y-16">
            {GROUPS.map((g) => (
              <div key={g.label}>
                <h2 className="text-xs font-semibold uppercase tracking-[0.14em] text-emerald-400">{g.label}</h2>
          ...
          <div className="mt-14 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
            <div className="grid gap-4 md:grid-cols-[auto_1fr] md:items-center">
              <div className="text-[15px] font-semibold text-white">Can&rsquo;t see your tool?</div>
              <div className="grid gap-3 text-[13px] leading-relaxed text-white/65 sm:grid-cols-3">
                <div>
                  Public REST API — see{" "}
                  <Link href="/developers" className="font-medium text-emerald-300 underline decoration-emerald-200 underline-offset-2 hover:decoration-emerald-500">
                    Developers
                  </Link>
                </div>
        ...
        <section className="mx-auto max-w-[1200px] px-6 pb-24">
          <div className="grid gap-6 rounded-2xl border border-white/10 bg-white/[0.03] p-8 md:grid-cols-[1fr_auto] md:items-center">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link href="/developers" className="inline-flex items-center rounded-lg border border-white/15 bg-white/[0.03] px-5 py-2.5 text-sm font-medium text-white/75 transition-colors hover:border-white/30 hover:bg-white/[0.03]">
                Read API docs
              </Link>
            </div>
          </div>
